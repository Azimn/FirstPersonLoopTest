#!/usr/bin/env python3
"""Subjective Character Loop v0.3 command-line entry point."""
...
from loopcore import CharacterLoop, PROBE_SYSTEM
...
Developer console commands:
  ...
  /thought                    begin one semantic private-thought cycle
...
def interactive(loop: CharacterLoop, interlocutor: str) -> None:
    print("Subjective Character Loop v0.3")
    print("Character-accessible state is first-person natural language only.")
    print("Private-thought duration is selected by a hidden semantic continuation probe.")
    print_help()
    print()
    while True:
        try:
        ...
        try:
            ...
            if cmd == "/help":
                ...
                loop.cognitive_cycle(trigger="manual")
            elif cmd == "/interest":
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Minimal first-person subjective character loop v0.3")
    ...
    db_path = Path(args.db) if args.db else Path(f"{args.character}_subjective_loop_v03.sqlite3")
...
__all__ = [
    "CharacterLoop", "HiddenState", "KIKI_IDENTITY", "ModelBackend", "OllamaBackend",
    "PRETORIUS_IDENTITY", "PROBE_SYSTEM", "ScriptedBackend", "select_identity",
]
