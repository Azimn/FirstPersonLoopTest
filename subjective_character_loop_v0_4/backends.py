...
class ScriptedBackend:
    """Deterministic shell backend exercising rest, thought, continuation, speech, and silence."""
    ...
    def __init__(self) -> None:
        ...
        self.probe_count = 0
        self.speech_count = 0
        self._init_script = ["THINK", "REST", "THINK", "THINK", "REST"]
        self._probe_script = ["CONTINUE", "RELEASE", "RELEASE", "CONTINUE", "CONTINUE", "RELEASE"]

    def complete(self, system: str, user: str, temperature: float = 0.8, max_tokens: int = 160) -> str:
        low = system.lower()
        if "hidden thought-initiation judgment" in low:
            value = self._init_script[self.init_count % len(self._init_script)]
            self.init_count += 1
            return value
