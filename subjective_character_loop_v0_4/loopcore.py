...
self-description, recent first-person awareness, and most recent private thought.
...
produce another immediate thought. Otherwise choose RELEASE. RELEASE means only that
attention rests for now, not that the matter is solved or forgotten.
...
class SubjectiveIngressGate:
    """One provenance-aware boundary for anything gaining subjective/autobiographical authority."""
    ...
    _telemetry = re.compile(
        ...
        r"trust|threat|salience|activation|score|variable)\s*[:=]\s*-?\d+(?:\.\d+)?\b",
        re.IGNORECASE,
    )
    _control = re.compile(r"^\s*(THINK|REST|CONTINUE|RELEASE)\s*$", re.IGNORECASE)
...
class CharacterLoop:
    ...
    def _probe_prompt(self, latest_thought: str) -> str:
        ...
        return f"This is who I understand myself to be:\n{self.identity}\n\nRecent first-person awareness:\n{context}\n\nMost recent private thought:\n{latest_thought}"

    def _accept_subjective(self, kind: str, text: str, provenance: str) -> bool:
        reason = self.ingress.reject_reason(provenance, text)
        if reason:
            self.journal.developer("ingress_rejected", f"provenance={provenance}; reason={reason}; text={text}")
            return False
    ...
    def _append_experience(self, text: str, provenance: str = "runtime_experience") -> bool:
        text = text.strip()
        if not text or not self._accept_subjective("experience", text, provenance):
            return False
    ...
    def hear(self, speaker: str, words: str, think: bool = True) -> None:
        ...
        self._append_experience(f'I hear {speaker} say, "{words}"', provenance="external_speech")
    ...
    def experience_opaque_action(self, action_phrase: str, think: bool = False) -> None:
        ...
        if not action_phrase:
            return
        if self._append_experience(f"I find myself {action_phrase}.", provenance="opaque_action") and think:
    ...
    def _generate_private(self, continuing: bool) -> str:
        ...
        for attempt in range(2):
            ...
            reason = self.ingress.reject_reason("private", text)
            if text and not reason:
                self.journal.add("thought", text)
                print(f"  thought:    {text}")
                return text
            self.journal.developer("private_rejected", f"reason={reason or 'empty'}; text={text or '<empty>'}")
        return ""

    def _initiation_decision(self) -> str:
        raw = self.backend.complete(INITIATION_SYSTEM, self._initiation_prompt(), temperature=0.0, max_tokens=self.probe_tokens).strip().upper()
    ...
    def cognitive_cycle(self, trigger: str = "time", force: bool = False) -> list[str]:
        if not force and self._initiation_decision() == "REST":
            if self.debug:
                print("  private:    [quiet]")
            return []
        thoughts: list[str] = []
    ...
    def _consider_outward_behavior(self, latest_thought: str) -> None:
        ...
        if self.allow_movement:
            ...
            if action:
                ...
                reason = self.ingress.reject_reason("action", action_experience)
                if reason:
    ...
    def _validate_spoken(self, spoken: str, private_thought: str, involuntary: bool = False) -> str:
        ...
        if not spoken:
            return ""
        reason = self.ingress.reject_reason("involuntary_speech" if involuntary else "speech", spoken)
        if reason:
            self.journal.developer("speech_rejected_ingress", f"reason={reason}; text={spoken}")
            return ""
    ...
    def show_hidden(self) -> str:
        return json.dumps({
            "hunger": round(self.state.hunger, 1), "fatigue": round(self.state.fatigue, 1),
            "pain": round(self.state.pain, 1), "temperature_discomfort": round(self.state.temperature_discomfort, 1),
            "boredom": round(self.state.boredom, 1), "interest": round(self.state.interest, 1),
            "interest_subject": self.state.interest_subject, "seconds_elapsed": round(self.state.seconds_elapsed, 1),
        }, indent=2)
