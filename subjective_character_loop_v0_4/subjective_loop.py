#!/usr/bin/env python3
"""Subjective Character Loop v0.4.4 command-line entry point."""
...
def interactive(loop: CharacterLoop, interlocutor: str) -> None:
    print("Subjective Character Loop v0.4.4")
    ...
    print("Any private thought visible to speech is inside the same copy-protection domain.")
    print_help()
    print()
    while True:
        try:
...
def main(argv: Optional[list[str]] = None) -> int:
    parser = argparse.ArgumentParser(description="Minimal first-person subjective character loop v0.4.4")
    ...
    db_path = Path(args.db) if args.db else Path(f"{args.character}_subjective_loop_v044.sqlite3")
